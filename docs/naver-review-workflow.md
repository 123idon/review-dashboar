...
부매니저 계정의 리뷰 화면에서 공개된 DOM의 본문·별점·날짜·리뷰번호를 읽는 경로를 확인했습니다. 엑셀 다운로드와 다른 계정은 필요하지 않습니다. 9/17 116건, 9/18 12건을 수동 브라우저 작업으로 복구했습니다. 이것은 서버 자동 수집 성공이 아닙니다.

`naver_seller.py`는 서버 직접 로그인 연결 화면과 동일한 DOM 기반 수집기입니다. 연결 기능은 기본 비활성화입니다. 로그인 정보 복사나 기존 PC 인증 쿠키 전송 기능은 없습니다.

## 활성화 전 필요한 사용자 승인

현재 판매자 계정의 로그인 상태를 Railway 비공개 영역에 지속 저장해 매일 리뷰 조회에 사용하는 접근 권한 승인이 필요합니다. 이전 계정 보호조치 이력이 있으므로 로그인/추가 인증은 사용자가 서버 연결 화면에서 직접 합니다. 보호조치/차단이 나타나면 중단하며 회피하지 않습니다. 브라우저 CPU/메모리는 기존 Railway 서비스 자원을 사용합니다.

승인 후 256비트 난수 연결 코드를 만들고 SHA256만 NAVER_SELLER_CONNECT_SHA256에, 1회 연결 작업에 필요한 짧은 만료 시각을 NAVER_SELLER_CONNECT_EXPIRES에 설정합니다. 연결 원문은 저장소·로그에 넣지 않으며 URL fragment로만 전달합니다. 페이지는 fragment를 즉시 지우고 명령 요청 헤더에만 사용합니다. 서버 브라우저는 15분 후 닫힙니다. 연결 완료 후 환경변수의 해시·만료 설정을 제거해 재연결 콘솔을 비활성화합니다.

로그인 상태는 `/app/data/naver_private/seller-state.json`에 0600으로 저장합니다. 비밀번호와 OTP는 파일이나 로그로 보관하지 않습니다. 전체 조회 건수와 고유 ID 수가 일치하고 실제 후기 검증·병합에 성공해야 연결 완료로 처리합니다. 연결 후 기존 자정 예약이 seller 모드로 실행되고 PC는 필요하지 않습니다. 인증 만료 시 기존 후기는 보존하고 상태에 실패를 표시합니다. 현재 서버 실수집은 인증 전이므로 검증되지 않았습니다.

연결 화면: `/naver-seller-connect` (승인된 단기 연결 코드 없이는 모든 작업 403)

아래는 이전 경로 기록입니다.

# 현재 운영 방식: 공개 후기 수집 (로그인 폐지)

계정 보호조치 신고 이후 판매자 로그인 기반 자동 수집은 폐지했습니다.
`/api/naver-automation/connect`는 본문을 읽지 않고 410을 반환합니다.
기존 연결 프로그램을 실행하지 마세요. 아래 판매자 로그인 관련 기록은 과거 이력입니다.

공개 수집기는 백년화편 공개 스토어에서 전체 상품 목록과 최신순 리뷰를 순회합니다.
사이트가 자체적으로 반환한 공개 리뷰 응답만 읽으며, 비공개 API 주소를 추측해 호출하지 않습니다.
로그인·인증 쿠키·프록시 회전·탐지 회피 기능은 없습니다.

중요: 공개 페이지 실제 응답 구조를 아직 확보하지 못하여 라이브 수집은 검증 전입니다.
어댑터 테스트는 합성 데이터로 검증한 것이며 실제 네이버 수집 성공을 의미하지 않습니다.
상품 개수 일치, 상품번호 일치, 리뷰 최신순, 수집 범위 종료를 확인한 경우만 병합합니다.
형식 불일치나 일부 상품 누락을 후기 0건이나 성공으로 처리하지 않습니다.

429/403/401 응답이나 접근 제한 화면이면 해당 실행을 중단합니다.
Retry-After와 최소 24시간 중 더 긴 기간을 기다리며 상태를 영구 저장합니다.
robots.txt가 허용하지 않으면 중단합니다. 레이아웃 변경·불완전 수집은 검토 전까지 중단합니다.
이미 관찰한 429의 보류 시각은 NAVER_PUBLIC_NOT_BEFORE에 설정합니다.
매일 Asia/Seoul 00:00 예약은 유지하며 보류 기간에는 네이버 요청을 하지 않습니다.
