...
async def get_goods_nos(client: httpx.AsyncClient, cat_url: str) -> list:
    goods_nos = []
    page = 1
    while True:
        try:
            ...
            print(f"  [자사몰] 상품목록 수집 실패 p{page}: {e}")
            break
    return goods_nos
...
def parse_jasaol_review(row, product_name: str):
    try:
        ...
        if star_tds:
            score = star_tds[0].get_text().count("★")
        content = ""
        for td in tds:
...
async def get_product_name(client: httpx.AsyncClient, goodsno: int) -> str:
    try:
        resp = await client.get(
            f"{JASAOL_BASE}/shop/goods/goods_view.php?goodsno={goodsno}", timeout=15)
        soup = fetch_html_euckr(resp)
...
async def scrape_jasaol(progress_cb=None) -> list:
    ...
    async with httpx.AsyncClient(
        ...
        for idx, goodsno in enumerate(all_goods):
            ...
            if progress_cb:
                progress_cb({
                    "phase": "detail", "done": idx + 1, "total": len(all_goods),
