...
function dailyPeriodLabel(r){
  return r.period_days===3?`${r.date_from} ~ ${r.date_to} · 금·토·일 통합`:r.date;
}
function dailyDateOption(d){
  const end=new Date(d+'T00:00:00Z');
  if(end.getUTCDay()!==0)return d;
  const start=new Date(end);start.setUTCDate(end.getUTCDate()-2);
  return start.toISOString().slice(0,10)+' ~ '+d+' (금·토·일)';
}
function dailyPaper(r){
  ...
  return `<div class="daily-head"><div><div class="daily-eyebrow">DAILY REVIEW BRIEF</div><div class="daily-title">전일 종합</div><div class="daily-date">${dailyEscape(dailyPeriodLabel(r))} · 자사와 경쟁사</div></div><div class="daily-gen">매일 오전 9시 갱신 · 한국시간<br>생성 ${dailyEscape(gen)}</div></div>
  <div class="daily-kpis"><div class="daily-kpi"><strong>${r.total.toLocaleString()}</strong>수집된 기간 내 후기</div><div class="daily-kpi"><strong>${r.low_count.toLocaleString()}</strong>3점 이하 후기</div><div class="daily-kpi"><strong>${r.selected_count.toLocaleString()}</strong>선정 표시 후기</div></div>
  <div class="daily-notice">${dailyEscape(r.notice)}</div>${dailyStatistics(r.statistics)}<div class="daily-grid">${r.brands.map(b=>`<section class="daily-card ${b.role==='자사'?'own':''}"><div class="daily-card-head"><div class="daily-brand">${dailyEscape(b.name)}<span class="daily-role">${dailyEscape(b.role)}</span></div><div class="daily-stats"><span>전체 수집 <b>${b.count??'—'}</b></span><span>선정 <b>${b.selected_count}</b></span><span>평점 <b>${b.average===null?'—':b.average.toFixed(2)}</b></span><span>3점 이하 <b>${b.low_count??'—'}</b></span></div></div><div class="daily-coverage">${dailyEscape(b.coverage)}<br>최신 보유 후기: ${dailyEscape(b.latest_review_date||'미확인')}</div>${b.reviews.length?b.reviews.map(v=>`<div class="daily-review"><div class="daily-review-top"><span class="daily-score ${v.score!==null&&v.score<=3?'low':''}">${v.score===null?'평점 미제공':v.score+' / 5'}</span><span>${r.period_days===3?dailyEscape(v.date)+' · ':''}${dailyEscape(({smartstore:'네이버',naver:'네이버',direct:'자사몰',kakao:'카카오'})[v.platform]||v.platform)}</span></div><div class="daily-product">${dailyEscape(v.product)}</div><p class="daily-quote">${v.excerpt?dailyHighlighted(v):'본문이 없는 후기입니다.'}</p></div>`).join(''):`<div class="daily-empty">${b.available?'이 기간에 부정·고객 불만 또는 기존 선정 기준에 해당하는 후기가 없습니다.<br>전체 수집 건수와 수집 상태를 함께 확인해 주세요.':'아직 수집되지 않는 브랜드입니다.<br>후기 0건으로 집계하지 않습니다.'}</div>`}</section>`).join('')}</div><div class="daily-foot">${dailyEscape(r.selection_rule)}<br>${dailyEscape(r.highlight_rule)}<br>백년화편 후기 대시보드 · 본 보고서는 수집 시점의 자료를 보존합니다.</div>`;
}
async function renderDailyReport(selectedDate){
  ...
  try{
    ...
    const options=dates.map(d=>`<option value="${dailyEscape(d)}" ${d===r.date?'selected':''}>${dailyEscape(dailyDateOption(d))}</option>`).join('');
    mc.innerHTML='<div class="daily-tools"><div><h2>전일 종합</h2><p>백년화편·명가삼대떡집 부정·고객 불만 포함 · 글자 수 많은 순 · 본문 전체 표시</p></div><div class="daily-actions"><label for="dailyDate">후기 날짜</label><select id="dailyDate" onchange="renderDailyReport(this.value)">'+options+'</select><button class="refresh-btn" onclick="dailySelectedDate=null;renderDailyReport()">최신 보고서</button><button class="refresh-btn" id="dailyDownload" onclick="downloadDailyReport()">전체 이미지 저장</button></div></div><p class="daily-archive-note">'+(r.reconstructed?'이전 보고서는 현재 보유한 해당 기간 원문에서 같은 선정 기준으로 표시합니다.':r.historical?'보관된 과거 원문에 현재와 같은 선정·강조 기준을 적용합니다.':'현재 보고서입니다. 월요일에는 금·토·일 후기를 합산합니다. 원본 후기는 보존하고 3점 이하·고객 불만 및 기존 선정 후기를 표시합니다.')+' 저장된 날짜 '+dates.length+'개 · 선정 후기 전체를 긴 PNG 이미지 한 장으로 저장합니다.</p><div id="dailyPaper" class="daily-paper">'+dailyPaper(r)+'</div>';
...
async function downloadDailyReport(){
  ...
  try{
    ...
    url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`전일종합_${exportReport.period_days===3?exportReport.date_from+'_':''}${exportReport.date}_전체.png`;
