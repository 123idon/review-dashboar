...
function dailyHighlighted(review){
  ...
  for(const h of review.highlights||[]){
    ...
    out+=dailyEscape(chars.slice(at,h.start).join(''))+`<mark class="negative">`+dailyEscape(chars.slice(h.start,h.end).join(''))+'</mark>';at=h.end;
  }
...
function dailyPaper(r, summaryOnly=false){
  if(summaryOnly){
    r={...r,brands:r.brands.map(b=>({...b,reviews:b.reviews.slice(0,3).map(v=>{
      const chars=Array.from(v.excerpt||'');const start=Math.max(0,(v.highlights?.[0]?.start||0)-25);
      const end=Math.min(chars.length,start+220);const prefix=start?'…':'';
      return {...v,excerpt:prefix+chars.slice(start,end).join('')+(end<chars.length?'…':''),highlights:(v.highlights||[]).filter(h=>h.start>=start&&h.end<=end).map(h=>({...h,start:h.start-start+prefix.length,end:h.end-start+prefix.length}))};
