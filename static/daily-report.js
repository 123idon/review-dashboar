...
function dailyAnalysis(a){
  if(!a||a.status!=='ready')return `<section class="daily-analysis"><h3>후기 해석 · 확인할 업무</h3><p>${dailyEscape(a?.message||'이 날짜의 저장된 AI 분석이 없습니다.')}</p><small>매일 오전 9시 5분 생성 · 한국시간</small></section>`;
  return `<section class="daily-analysis"><h3>후기 해석 · 확인할 업무</h3><p class="daily-analysis-summary">${dailyEscape(a.summary)}</p>${(a.findings||[]).map(f=>`<article><h4><span>${dailyEscape(({jasaol:'백년화편',myeongga:'명가삼대떡집'})[f.brand]||f.brand)}</span> ${dailyEscape(f.title)}</h4><p>${dailyEscape(f.meaning)}</p><div class="daily-analysis-action"><b>확인할 업무</b> ${dailyEscape(f.action)}</div>${f.evidence.map(e=>`<blockquote>${dailyEscape(e.quote)}</blockquote>`).join('')}</article>`).join('')}<small>AI 해석${a.reviewed_at?' · 원문 대조 검수 완료':''} · 선정 후기 ${a.source_count}건 기준 · 전체 고객 의견이나 원인 확정이 아닙니다.<br>분석 ${dailyEscape(a.generated_at?new Date(a.generated_at).toLocaleString('ko-KR',{timeZone:'Asia/Seoul',hour12:false}):'—')} · 저장된 결과를 표시하며 조회·이미지 저장 시 재분석하지 않습니다.</small></section>`;
}
