...
async function renderDailyReport(){
  ...
  const mc=document.getElementById('mainContent');mc.innerHTML='<div class="daily-tools"><div><h2>전일 종합</h2><p>보고서를 불러오고 있습니다.</p></div></div>';
  try{
    const response=await fetch('/api/daily-report');if(!response.ok)throw new Error('보고서를 생성하지 못했습니다. 잠시 후 다시 확인해 주세요.');
    const r=await response.json();if(currentShop!=='daily')return;dailyReportData=r;
    mc.innerHTML='<div class="daily-tools"><div><h2>전일 종합</h2><p>오전 9시 자동 갱신 · 수집된 원문 발췌 · 형광펜 표시</p></div><button class="refresh-btn" id="dailyDownload" onclick="downloadDailyReport()">한 장 이미지 저장</button></div><div id="dailyPaper" class="daily-paper">'+dailyPaper(r)+'</div>';
  }catch(e){if(currentShop==='daily')mc.innerHTML='<div class="card">'+dailyEscape(e.message)+'</div>';}
}
async function downloadDailyReport(){
  if(!dailyReportData)return;const button=document.getElementById('dailyDownload');button.disabled=true;button.textContent='이미지 만드는 중…';
  const stage=document.createElement('div');stage.className='daily-paper daily-export';stage.style.cssText='position:absolute;left:-12000px;top:0;';stage.innerHTML=dailyPaper(dailyReportData);document.body.appendChild(stage);
  try{await document.fonts.ready;if(typeof html2canvas!=='function')throw new Error('이미지 저장 모듈을 불러오지 못했습니다. 페이지를 새로고침해 주세요.');
    ...
    const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='전일종합_'+dailyReportData.date+'.png';a.click();setTimeout(()=>URL.revokeObjectURL(url),10000);
